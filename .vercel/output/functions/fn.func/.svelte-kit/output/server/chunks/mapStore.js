import { c as create_ssr_component, b as subscribe, a as add_attribute, j as get_store_value, o as onDestroy, e as escape, v as validate_component } from "./ssr.js";
...
const random = (map, location) => Math.floor(Math.random() * 5);
...
const up = (map, location) => (reader = type) => location - map.rows >= 0 && reader(map.layers.ground, location - map.rows) === reader(map.layers.ground, location);
const down = (map, location) => (reader) => location + map.rows < map.layers.ground.length && reader(map.layers.ground, location + map.rows) === reader(map.layers.ground, location);
const left = (map, location) => (reader) => location % map.rows !== 0 && reader(map.layers.ground, location - 1) === reader(map.layers.ground, location);
const right = (map, location) => (reader) => (location + 1) % map.rows !== 0 && reader(map.layers.ground, location + 1) === reader(map.layers.ground, location);
...
const Scroller = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let { cellWidth: cellWidth2 = 60 } = $$props;
  let { cellHeight: cellHeight2 = 60 } = $$props;
  let { rows = 10 } = $$props;
  let { cols = 10 } = $$props;
  let { handleClick = (x, y) => {
  } } = $$props;
  ...
  let { handleOffset = (x, y, zoom) => {
  } } = $$props;
  let { handleKeypress = (key, shiftKey) => {
  } } = $$props;
  let container;
  let content;
  ...
  if ($$props.cellWidth === void 0 && $$bindings.cellWidth && cellWidth2 !== void 0)
    $$bindings.cellWidth(cellWidth2);
  if ($$props.cellHeight === void 0 && $$bindings.cellHeight && cellHeight2 !== void 0)
    $$bindings.cellHeight(cellHeight2);
...
const TileSelector = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<section class="grid flex-grow overflow-clip"><div class="col-start-1 row-start-1 cursor-pointer">${slots.default ? slots.default({
    ...
    cellWidth,
    cellHeight
  }) : ``}</div> <div class="col-start-1 row-start-1 pointer-events-none relative"><div class="absolute border-2 border-red-500"${add_attribute("style", selectedStyles, 0)}></div> <div class="absolute bg-yellow-500 opacity-30"${add_attribute("style", hoveredStyles, 0)}></div></div></section>`;
});
const Game = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let validTile = (x, y) => x < rows && y < cols;
  const interfacer = (() => {
    return {
      selected: { x: -1, y: -1 },
      hover: { x: -1, y: -1 },
  ...
  let cols = map.cols;
  let renderData = {
    ground: (type2) => get_store_value(rendererStore).ground[type2],
    unit: (type2) => typeof type2 !== "undefined" ? get_store_value(rendererStore).units[type2] ?? null : null,
    sky: (type2) => typeof type2 !== "undefined" ? get_store_value(rendererStore).sky[type2] ?? null : null
  };
...
const renderObject = (render, object) => (width, height, animationFrame2) => (context) => context.drawImage(
  ...
  animationFrame2 % render.frames * (height + render.yOffset),
...
const paint = (renderData) => (map) => (context) => (row, col, left2, top, width, height, zoom) => {
  context.save();
...
const MapRender = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-115ss56_START -->${$$result.title = `<title>${escape(PUBLIC_GAME_NAME)}</title>`, ""}<!-- HEAD_svelte-115ss56_END -->`, ""} <div class="flex gap-2 border-4 border-black h-full bg-stone-400">${validate_component(Game, "Game").$$render($$result, { map, makeImage, select }, {}, {
    default: ({ select: select2, interfacer, validTile, rows, cols, renderData }) => {
      return `${loaded ? `${validate_component(TileSelector, "TileSelector").$$render($$result, { interfacer, select: select2, validTile }, {}, {
        default: ({ handleClick, handleHover, handleKeypress, handleOffset, cellWidth: cellWidth2, cellHeight: cellHeight2 }) => {
...
export {
  MapRender as M,
  createImageLoader as a,
  connectionDecision as c,
  loadedState as l,
  mapStore as m,
  terrainData as t,
  unitData as u
