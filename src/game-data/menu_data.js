...
/*** Map Editor ***/
let MapEditorClass = function (__saved_imgs) {
	...
	const set_current_interactions = function (list) {
		for (let i = 0; i < 8; i++) {
			if (list[i] != null) current_interactions[i] = list[i]
		}
	}
	...
	const Set_Controls = function (handler) {
		if (last_handler != null) {
			return
			last_handler.removeEventListener(
				'touchstart touchmove touchend touchcancel mousedown mouseup contextmenu mousemove'
			)
			last_handler = null
		}
	...
	function send_map_data_to_server(type, data) {
		if (!online) return
		if (SERVER.new_map && type == 'update') type = 'upload'
		socket.emit('mapdata ' + type, socket.password, data)
	}
	function display_server_saved_maps(replace_old, create_new) {
		Clean_Popup_Screen()
		...
		SERVER.onReportGameList(function (_list_data) {
			...
			for (let _m in _list_data) {
				try {
					...
					if (_read_game_data[index].Valid) {
						setTimeout(
							function (index) {
								...
								Canvas.ScaleImageData(
									ctx,
									INTERFACE.Get_Sample(sampledGame),
									0,
									0,
									10 / sampledGame.Terrain_Map.Width,
									10 / sampledGame.Terrain_Map.Height
								)
								...
								delete_btn.onclick = function () {
									...
									question.innerHTML =
										'Do you really want to delete ' + _read_game_data[index].Name + '?'
					...
					let index = _list_data[_m].saveindex
					...
					setTimeout(
						function (index, id) {
							...
							delete_btn.onclick = function () {
								...
								question.className = 'editor-border w3-sand'
								question.innerHTML =
									'This map is corrupted, but doing this will permanently delete it. Try and content us to see if we can salvage your data first.<br /><br />Really delete it?'

								yes_btn = document.createElement('div')
								yes_btn.className = 'w3-button w3-green w3-inline'
								yes_btn.innerHTML = 'Yes'
								yes_btn.onclick = function () {
									send_map_data_to_server(SERVER.DELETE, id)
								}
							...
							sadface.className = 'fas fa-sad-tear'
							_casings[index].appendChild(sadface)
							_casings[index].appendChild(document.createElement('div'))
							_casings[index].appendChild(delete_btn)
						},
						15,
						index,
						_list_data[_m].map_id
					)
				}
			}
			...
			for (let i = 0; i < 9; i++) {
				...
				casing.onclick = function () {
					if (_read_game_data[i] == null) {
						if (create_new != null) {
							if (DATA.name == 'Unnamed Custom Map') {
								LOG.prompt(
									'Give your map a name',
									function (result) {
										SERVER.local_saved_map = i
										DATA.name = result
						...
						if (replace_old) {
							// This will remove the map. Is that okay?
							let question, yes_btn, no_btn
							Clean_Popup_Screen(container)

							question = document.createElement('p')
							question.className = 'editor-border w3-sand'
							question.innerHTML =
								'This will delete the map ' +
								_read_game_data[i].Name +
								'.<br /><br />Delete and Replace?'
	...
	function map_img() {
		let sampledGame = new Engine_Class(Map_Reader.String(map_data_to_str()), true)
		sampledGame.Set_Interface(INTERFACE)
		sampledGame.FORCE_MERGE_DISPLAY = true
	...
	function map_data_to_str() {
		...
		str += DATA.width + ';'
		str += DATA.height + ';'
		for (let i = 0; i < DATA.map_list.length; i++) str += DATA.map_list[i] + ':'
		str += ';'
		for (let i = 0; i < DATA.max_players; i++) str += DATA.players[i] + ':'
		str += ';'
		for (let i = 0; i < DATA.units.length; i++) str += DATA.units[i] + ':'
		str += ';'
		for (let i = 0; i < DATA.cities.length; i++) str += DATA.cities[i] + ':'
		str += ';'
	...
	function make_map_smaller(new_x, new_y, start_x, start_y) {
		...
		for (let i = DATA.units.length - 1; i >= 0; i--) {
			DATA.units[i][1] -= start_x
			DATA.units[i][2] -= start_y
			if (DATA.units[i][1] < 0 || DATA.units[i][2] < 0) DATA.units.splice(i, 1)
		...
		for (let i = DATA.cities.length - 1; i >= 0; i--) {
			DATA.cities[i][1] -= start_x
			DATA.cities[i][2] -= start_y
			if (DATA.cities[i][1] < 0 || DATA.cities[i][2] < 0) DATA.cities.splice(i, 1)
			else if (DATA.cities[i][1] >= DATA.width || DATA.cities[i][2] >= DATA.height)
				DATA.cities.splice(i, 1)
		}
	}
	function Change_Map_Size(x, y, start_x, start_y) {
		...
		if (DATA.map_list == null) {
			...
			if (start_x == null) start_x = 0
			if (start_y == null) start_y = 0
			if (x > DATA.width) make_map_bigger(x, DATA.height, start_x, 0)
			if (x < DATA.width) make_map_smaller(x, DATA.height, start_x, 0)
			if (y > DATA.height) make_map_bigger(DATA.width, y, 0, start_y)
			if (y < DATA.height) make_map_smaller(DATA.width, y, 0, start_y)
		}
	}
	...
	function remove_player(PLAYER, callback) {
		...
		question.className = 'editor-border w3-sand'
		question.innerHTML =
			'This will remove all of Player ' +
			(PLAYER + 1) +
			"'s units and cities. It won't change the script.<br />Make sure to check your script after to make sure it still works!<br /><br />Are you okay with this?"
		...
		yes_btn.onclick = function () {
			DATA.max_players--
			for (let i = DATA.units.length - 1; i >= 0; i--) {
				if (DATA.units[i][3] == PLAYER) DATA.units.splice(i, 1)
				else if (DATA.units[i][3] > PLAYER) DATA.units[i][3]--
			}
			for (let i = DATA.cities.length - 1; i >= 0; i--) {
				if (DATA.cities[i][3] == PLAYER) DATA.cities.splice(i, 1)
				else if (DATA.cities[i][3] > PLAYER) DATA.cities[i][3]--
			}
	...
	function change_active_player(PLAYER) {
		...
		document.getElementById('editor-color-changer0').style.backgroundColor = data_to_hex(
			Team_Colors.Color[ACTIVE_PLAYER + 1][0]
		)
		document.getElementById('editor-color-changer1').style.backgroundColor = data_to_hex(
			Team_Colors.Color[ACTIVE_PLAYER + 1][0]
		)
	...
	function Clean_Popup_Screen(container) {
		if (container == null) container = document.getElementById('e-p-content')
	...
	function Open_Players_Editor() {
		...
		for (let i = 0; i < LIST_CONTAINER.childNodes.length; ) LIST_CONTAINER.childNodes[0].remove()
	...
	function Open_Weather_Editor() {
		...
		let __weather = [
			Images.Retrieve('Sunny Icon'),
			Images.Retrieve('Rainy Icon'),
			Images.Retrieve('Snowy Icon'),
			Images.Retrieve('Heat Icon'),
		]
		let __names = ['Sunny', 'Rainy', 'Snowy', 'Dry Heat']
		let turn = DATA.weather.length - 1
		const STATIC_RATE = 1
		let __cur_data = new Array(turn),
			__weather_pie_data
		let __start_turn = 0
		...
		function display_chart() {
			...
			new Chart(__PIE_CHART.getContext('2d'), {
				// pie chart
			...
			new Chart(__TIME_CHART.getContext('2d'), {
				// pie chart
	...
	function Open_Options() {
		...
		/// 1) allows you to rename the map

		let rename_map = document.createElement('div')
		rename_map.className = 'editor-options-list'

		caption = document.createElement('h3')
		caption.className = 'editor-text w3-rest w3-left-align'
		caption.style.margin = '0px'
		caption.innerHTML = 'Rename ' + DATA.name
		const RENAME_POINTER = caption
		...
		button.onclick = function () {
			LOG.prompt(
				"Change the name from '" + DATA.name + "'?",
				function (result) {
					DATA.name = result
					RENAME_POINTER.innerHTML = 'Rename ' + DATA.name
					Edited()
				},
				{
					min: 3,
		...
		rename_map.appendChild(button)
		rename_map.appendChild(caption)
		...
		container.appendChild(rename_map)
	...
	let SAVED_UNIT_IMAGES = __saved_imgs.units
	let SAVED_PLACE_IMAGES = __saved_imgs.places
	function refresh_choice_images() {
		let image_list = new Array(11)
		if (TYPE == CHOICE.TERRAIN) {
			let list = Terrain_Data.TERRE
			for (let i = 1; i < list.length; i++) {
				image_list[i] = list[i].Sprite[0].Source()
			}
		} else if (TYPE == CHOICE.UNITS) {
			let list = Char_Data.CHARS
			for (let i = 1; i < list.length; i++) {
				image_list[i] = SAVED_UNIT_IMAGES[ACTIVE_PLAYER + 1][i].Source()
			}
			...
			list = Building_Data.PLACE
			for (let i = 1; i < list.length; i++) {
				image_list[i] = SAVED_PLACE_IMAGES[ACTIVE_PLAYER + 1][i].Source()
			}
		}
		for (let i = 1; i < 11; i++) {
			if (image_list[i + 10 * PAGE] == null)
				document.getElementById('editor-selection' + i + 'img').src = BLANK
			else document.getElementById('editor-selection' + i + 'img').src = image_list[i + 10 * PAGE]
		}
	}
	function CHANGECHOICE(changeTo) {
		if (changeTo == CHOICE.TERRAIN) {
			...
			TYPE = CHOICE.WEATHER
			refresh_choice_images()
		} else if (changeTo == CHOICE.ERASE) TYPE = CHOICE.ERASE
	}
	function CHOOSEINDEX(index) {
		if (document.getElementById('editor-selection' + index + 'img').src == BLANK) return
	...
	function Allowed(_tile) {
		if (TYPE == CHOICE.UNIT) {
			...
			if (_ter.Type == 7)
				// units cannot exist on impassable terrain
				return false
			if (_ter.Type == 6)
				if (_unit.Type == 0)
					// ground units cannot traverse sea terrain
					return false
			if (_ter.Type == 2)
				if (_unit.Move_Type == 1 || _unit.Move_Type == 2)
					// only foot units can climb rugged terrain
					return false
			if (_ter.Type != 6)
				if (_unit.Type == 2)
					// sea units cannot traverse ground terrain
					return false
		} else if (TYPE == CHOICE.CITY) {
			var _city = Building_Data.PLACE[SELECTION]
			var _ter = Terrain_Data.TERRE[DATA.map_list[_tile]]
			if (_ter.Type == 7)
				// cities cannot be on impassable terrain
				return false
			if (_ter.Type == 2)
				// cities cannot be on rugged terrain
				return false
			if (_ter.Type == 6)
				if (_city.Terrain == 0)
					// ground cities can't exist on sea terrain
					return false
			if (_ter.Type != 6)
				if (_city.Terrain == 2)
					// sea cities can't exist on ground terrain
					return false
			...
			if (Terrain_Data.Get('Shore') == SELECTION) {
				...
				if (_tile < DATA.map_list.length) {
					...
					if (ter_check.Type == 6) sea = true
					else if (ter_check.Type < 6 && ter_check.Type >= 0) ground = true
				}
				if (_tile > 0) {
					ter_check = Terrain_Data.TERRE[DATA.map_list[_tile - 1]]
					if (ter_check.Type == 6) sea = true
			...
			if (_ter.Type == 6) {
				if (Terrain_Data.Get('Sea') != SELECTION) {
					...
					if (_tile < DATA.map_list.length)
						if (Terrain_Data.TERRE[DATA.map_list[_tile + 1]].Type != 6) return false
					if (_tile > 0) if (Terrain_Data.TERRE[DATA.map_list[_tile - 1]].Type != 6) return false
					if (_tile + height < DATA.map_list.length)
						if (Terrain_Data.TERRE[DATA.map_list[_tile + height]].Type != 6) return false
					if (_tile - height > 0)
						if (Terrain_Data.TERRE[DATA.map_list[_tile - height]].Type != 6) return false
				}
			} else if (_ter.Type == 8) {
	...
	function Edited() {
		...
		setTimeout(function () {
			// turn save button red after 5 minutes without saving
	...
	function Reflow() {
		scroller.setDimensions(
			...
			DATA.width * tile_size + 10,
			DATA.height * tile_size
		)
	}
	function paint(y, x, left, top) {
		//	inverting x & y on purpose
		let TILE = x * DATA.height + y
		let y_scale = Terrain_Data.TERRE[DATA.map_list[TILE]].Sprite[0].Image().height / TILESIZE,
			y_diff =
				(Terrain_Data.TERRE[DATA.map_list[TILE]].Sprite[0].Image().height - TILESIZE) / y_scale
		Terrain_Data.TERRE[DATA.map_list[TILE]].Sprite[0].Draw(
			CANVAS,
			left,
			top - y_diff,
			tile_size,
	...
	function render(left, top) {
		...
		let flag_str,
			current,
			shrink_amt = tile_size / 10
		for (let i = 0; i < DATA.cities.length; i++) {
			current = DATA.cities[i]
			SAVED_PLACE_IMAGES[current[3] + 1][current[0]].Draw(
				CANVAS,
				current[1] * tile_size + shrink_amt - LEFT,
				current[2] * tile_size + shrink_amt - TOP,
				tile_size - 2 * shrink_amt,
				tile_size - 2 * shrink_amt
			)
		}
		for (let i = 0; i < DATA.units.length; i++) {
			current = DATA.units[i]
			SAVED_UNIT_IMAGES[current[3] + 1][current[0]].Draw(
				CANVAS,
				current[1] * tile_size + shrink_amt - LEFT,
				current[2] * tile_size + shrink_amt - TOP,
				tile_size - 2 * shrink_amt,
	...
	function SELECT(x, y) {
		...
		if (TYPE == CHOICE.TERRAIN) {
			...
			LOG.popup(
				"Weather Tiles coming in next update! Things like Clouds / Storm / Tornados / etc. They will affect flying units, and stealth ground units, but they move with the wind and don't stay in the same place."
			)
		}
	...
	this.Save = function () {
		...
		if (SERVER.local_saved_map == -1) {
			...
			display_server_saved_maps(true, true)
			return
		}
	...
	this.Test = function () {
		...
		for (let i = 0; i < each_player.length; i++) each_player[i] = false
		for (let i in DATA.units) each_player[DATA.units[i][3]] = true
	...
	this.Upload = function () {
		...
		send_map_data_to_server(SERVER.PUBLISH, {
			pic: map_img(),
			// pic:encrypt_game_data(map_img()),
			id: DATA.id,
		})
	...
	this.Page_Right = function () {
		if (TYPE == CHOICE.TERRAIN) {
			if (PAGE * 10 + 11 >= Terrain_Data.TERRE.length) return
		} else if (TYPE == CHOICE.UNITS) {
			if (PAGE * 10 + 11 >= Char_Data.CHARS.length) return
		} else if (TYPE == CHOICE.CITY) {
			if (PAGE * 10 + 11 >= Building_Data.PLACE.length) return
		} else if (TYPE == CHOICE.WEATHER) {
			if (PAGE * 10 + 11 >= Weather_Data.WEATHER.length) return
		}
		PAGE++
		refresh_choice_images()
	}
	...
	this.Init = function () {
		...
		for (let i = 0; i < CONTAINER.childNodes.length; ) CONTAINER.childNodes[0].remove()
	...
	this.Open = function (save_data_index, game_data, tested_complete) {
		...
		} else document.getElementById('editor-upload-btn').style.display = 'none'
		...
		if (game_data == null || !game_data.Valid) {
			...
			for (let i = 0; i < DATA.max_players; i++) DATA.players[i] = DEFULT_NAMES[i]
			...
			if (game_data.uploaded) {
				LOG.popup(
					'Warning: This map is already uploaded. Editing and Saving ' +
						'this map will remove it from the published maps page. You will have ' +
						'to re-test it and re-publish it to put it back on the public server.'
				)
			}
		}
...
Menu.No_Touch_Overlay.Add(
	new Canvas.Drawable(
		new Text_Class('20pt Impact', '#FFF'),
		null,
		300,
		100,
		200,
		200,
		'Input stopped until user reconnects'
	)
)
Menu.No_Touch_Overlay.Add(
	new Canvas.Drawable(
		new Text_Class('15pt Impact', '#FFF'),
		null,
		300,
		300,
		200,
		200,
		'If 30 seconds pass, they will forfeit'
	)
)
...
Menu.PreGame.AddStarter = function (mapName) {
	...
	let HOLDER = document.getElementById('HOSTGAMEBUTTONS')
	for (let i = 2; i < HOLDER.childNodes.length; i++) {
		HOLDER.childNodes[i].remove()
	}
...
Menu.PreGame.Set = function (index, value) {
	...
	document.getElementById('HOSTGAMESTART').innerHTML = INTERFACE.Game.Full()
		? 'Start Game'
		: 'Start with AI'
}
...
Menu.PreGame.Setup_Map = function (map) {
	...
	for (let i = 0; i < playersAmount; i++) {
		...
		curEl.innerHTML =
			"<div class='w3-center lds-ellipsis' style='color:black;max-height:14px;'><div></div><div></div><div></div></div>"
