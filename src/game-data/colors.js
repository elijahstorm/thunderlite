...
var Team_Colors = {
	...
	Flags: [
		Images.Retrieve('White Flag'),
		Images.Retrieve('Red Flag'),
		Images.Retrieve('Blue Flag'),
		Images.Retrieve('Green Flag'),
		Images.Retrieve('Yellow Flag'),
	],
	Base: Images.Retrieve('Flag Base'),
	Draw: function (canvas, x, y, h, player) {
		if (player == null) player = this.Flags[0]
...
function data_to_hex(data, alpha) {
	...
	if (data.length != 4 && alpha != null) data[3] = alpha
	return (
		'#' +
		data[0].toString(16) +
		data[1].toString(16) +
		data[2].toString(16) +
		(data[3] == null ? '' : data[3].toString(16))
	)
}
...
function clone(img) {
	let temp = imageHolderCanvas.createImageData(img.width, img.height)
	for (let i = 0; i < img.data.length; i++) temp.data[i] = img.data[i]
	return temp
}
