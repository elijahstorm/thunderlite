<script lang="ts">
	...
	onMount(() => {
		...
		const timer = setInterval(() => (frame += 1), ANIMATION_TIME)
		return () => clearInterval(timer)
	})
</script>

<svelte:head>
	<title>ThunderLite — Units & FX</title>
</svelte:head>
...
<main class="min-h-screen space-y-12 bg-slate-900 p-6 text-slate-100">
	<header class="space-y-1">
		...
		<h1 class="text-2xl font-bold">Units &amp; FX</h1>
		<p class="text-sm text-slate-400">
			Animations straight from the real engine renderer. Dev server only.
		</p>
	</header>
