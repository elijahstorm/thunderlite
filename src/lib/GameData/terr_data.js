...
var Terrain_Data = {
	...
	Is_Reachable: function (terrain_type, unit_type) {
		if (terrain_type == 6) if (unit_type == 0) return false
		if (terrain_type == 7) return false
		if (unit_type == 2) return false
		if (unit_type == -1) return false
		return true
	},
	...
	Reverse_Get: function (index) {
		if (index < Terrain_Data.TERRE.length) return Terrain_Data.TERRE[index]
	},
}
...
for (var x = 1; x < Terrain_Data.TERRE.length; x++) {
	...
	if (_t.Name == 'Sea' || _t.Name == 'Shore') {
		...
		for (var j = 0; j < 11; j++) {
			_t.Borders[j] = Images.Declare(
				'Terrain/' + _t.Name + '/' + _t.Name + 'Border' + j + '.png',
				_t.Name + 'Border' + j + ''
			)
	...
	for (var i = 0; i < _t.Sprite.length; i++) {
		_t.Sprite[i] = Images.Declare(
			'Terrain/' + _t.Name + '/' + _t.Name + '' + i + '.png',
			_t.Name + i
		)
