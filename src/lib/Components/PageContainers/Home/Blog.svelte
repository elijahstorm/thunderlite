...
<section class="py-14 lg:py-24">
	<div class="container">
		<h2
			class="text-center text-3xl lg:text-4xl text-primary-dark-blue mb-5 lg:text-left lg:mb-10"
		>
			Latest Articles
		</h2>
			...
			{#each articles as article (article.title)}
				<article class="bg-white">
					<div class="aspect-w-16 aspect-h-10 lg:aspect-w-4 lg:aspect-h-3">
						<img class="object-cover" src={article.imgPath} alt={article.imgAlt} />
					</div>
					...
					<div class="px-7 pt-5 pb-10 lg:p-6">
						...
						<h4 class="text-primary-dark-blue text-sm py-2 hover:text-green-400">
							<a href={article.imgPath}>{article.title}</a>
						</h4>
