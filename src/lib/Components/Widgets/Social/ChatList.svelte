<script lang="ts">
	...
	const openSettings = () => (settingsOpen = !settingsOpen)
...
<section class="flex flex-col justify-center antialiased bg-gray-50 text-gray-600 min-h-screen p-4">
	<div class="h-full">
		<div class="relative max-w-[340px] mx-auto bg-white shadow-lg rounded-lg">
			<header class="relative pt-6 pb-2 px-5 border-b border-gray-200">
				{#await me}
					<Loader />
				{:then me}
					<div class="flex justify-between items-center mb-3">
						{#if me}
							<UserImageAndName user={me} text />
						{/if}
						<!-- Settings button -->
				...
				{/await}
				{#if settingsOpen}
					<button class="fixed inset-0 cursor-default" on:click={() => (settingsOpen = false)} />
					<!-- Dropdown menu -->
					<div
						class="absolute right-4 z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600"
			...
			<!-- Chat list -->
			<InfiniteScroll
				tailwind="py-3 px-5 max-h-96 h-96 overflow-y-auto"
				threshold={60}
				on:load={loadMoreUsers}
			>
