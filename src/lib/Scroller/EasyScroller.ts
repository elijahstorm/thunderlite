export let EasyScroller = function (content, options) {
...
EasyScroller.prototype.render = (function () {
	...
	if (helperElem.style[perspectiveProperty] !== undef) {
		return function (left, top, zoom) {
			this.content.style[transformProperty] =
...
document.addEventListener(
	...
	function () {
		...
		for (var i = 0; i < elements.length; i++) {
			element = elements[i]
			var scrollable = element.attributes.getNamedItem('data-scrollable')
				? element.attributes.getNamedItem('data-scrollable').value
				: null
			var zoomable = element.attributes.getNamedItem('data-zoomable')
